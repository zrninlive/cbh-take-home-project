import { describe, it, expect, beforeAll } from "vitest";

import {
  GenerateReportUseCaseErrors,
  GenerateReportUseCase,
} from "./GenerateReportUseCase";

import { AgentRepository } from "../../agents/entity";
import { FacilityRepository } from "../../facilities/entity";
import { ShiftRepository } from "../entity/repository";

const agentsRepository = new AgentRepository();
const facilitiesRepository = new FacilityRepository();
const shiftsRepository = new ShiftRepository();

const generateReportUseCase = new GenerateReportUseCase(
  agentsRepository,
  facilitiesRepository,
  shiftsRepository
);

const agentMockData = {
  id: "6f7eeb39-15d6-4c1a-8511-9c1c36609a46",
  name: "John Doe",
  document: "123456",
  createdAt: new Date(),
  modifiedAt: new Date(),
};

const facilityMockData = {
  id: "fc2b5cb6-c1af-11ed-afa1-0242ac120002",
  name: "FACILITY 01",
  code: "FAC-01",
  createdAt: new Date(),
  modifiedAt: new Date(),
};

const firstShiftMockData = {
  id: "776c12b8-c1b4-11ed-afa1-0242ac120002",
  agentId: "6f7eeb39-15d6-4c1a-8511-9c1c36609a46",
  facilityId: "fc2b5cb6-c1af-11ed-afa1-0242ac120002",
  startsAt: new Date("2023-03-13T10:00:00"),
  finishedAt: new Date("2023-03-13T14:00:00"),
  createdAt: new Date(),
  modifiedAt: new Date(),
};

const secondShiftMockData = {
  id: "7cd3002c-c1b4-11ed-afa1-0242ac120002",
  agentId: "6f7eeb39-15d6-4c1a-8511-9c1c36609a46",
  facilityId: "fc2b5cb6-c1af-11ed-afa1-0242ac120002",
  startsAt: new Date("2023-03-12T10:00:00"),
  finishedAt: new Date("2023-03-12T14:30:00"),
  createdAt: new Date(),
  modifiedAt: new Date(),
};

const thirdShiftMockData = {
  id: "7cd3002c-c1b4-11ed-afa1-0242ac120002",
  agentId: "6f7eeb39-15d6-4c1a-8511-9c1c36609a46",
  facilityId: "fc2b5cb6-c1af-11ed-afa1-0242ac120002",
  startsAt: new Date("2023-03-11T10:00:00"),
  finishedAt: new Date("2023-03-11T15:05:00"),
  createdAt: new Date(),
  modifiedAt: new Date(),
};

describe("Generate reports from shifts Tests", () => {
  beforeAll(() => {
    agentsRepository.create(agentMockData);

    facilitiesRepository.create(facilityMockData);

    shiftsRepository.create(firstShiftMockData);
    shiftsRepository.create(secondShiftMockData);
    shiftsRepository.create(thirdShiftMockData);
  });

  it("should not return shifts if the facilityId is invalid", () => {
    try {
      generateReportUseCase.execute({
        agentId: agentMockData.id,
        facilityId: "invalid-facilityId",
      });
    } catch (err) {
      expect(err.message).toBe(
        GenerateReportUseCaseErrors["facility-not-exists"]
      );
    }
  });

  it("should not return shifts if the agentId is invalid", () => {
    try {
      generateReportUseCase.execute({
        agentId: "invalid-agentId",
        facilityId: facilityMockData.id,
      });
    } catch (err) {
      expect(err.message).toBe(GenerateReportUseCaseErrors["agent-not-exists"]);
    }
  });

  it("should return agent, facility and shifts data", () => {
    const result = generateReportUseCase.execute({
      agentId: agentMockData.id,
      facilityId: facilityMockData.id,
    });

    expect(result).toEqual({
      agent: agentMockData,
      facility: facilityMockData,
      shifts: [firstShiftMockData, secondShiftMockData, thirdShiftMockData],
    });
  });
});
